...
import { FilterExcelExampleComponent } from './example/example';
import { CodeFile } from '../../../../interfaces/code-file';

@Component({
  selector: 'docs-core-features-filtering-excel',
  imports: [LiveExampleComponent, FilterExcelExampleComponent],
  templateUrl: './excel.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FilteringFilterExcelComponent {
  codeFiles: CodeFile[] = [
    {
      fileName: 'example.ts',
      code: `
          ...
          @Component({
            selector: 'docs-core-features-filtering-filter-excel-example',
            imports: [D_GRID],
            templateUrl: './example.html',
            changeDetection: ChangeDetectionStrategy.OnPush,
          })
          export class FilterExcelExampleComponent implements OnInit {
            gridData = signal<Row[]>([]);
    ...
    },
    {
      ...
      code: `
        <d-grid [style.height.%]="87" [data]="gridData()" [asyncData]="true" filterable="excel">
